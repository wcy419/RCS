function [feasible,chrom]=isfeasible(chrom0,n)
       ...
       if ismember(chrom(j),check)
           if chrom(j) ~= 1
                   if ~ismember(dual_feasible(chrom(j)),chrom(j+1:k_plus)) %拾取n-后如果在有n+(chrom(j)+1)操作则不违反。
                        ...
                        %disp(-1);
                        convert_chrom = convert(chrom,chrom(j),dual_feasible(chrom(j)));
                        chrom = convert_chrom;
                        chrom0 = convert_chrom;
%                         feasible = 1;
                   else
                       if chrom(j+1) ~= dual_feasible(chrom(j))&&chrom(j+1) ~= dual_feasible(minus(i))
                            convert_chrom = convert(chrom,chrom(j),dual_feasible(chrom(j)));
                            chrom = convert_chrom;
                            chrom0 = convert_chrom;
%                            feasible = 1;
