...
N = size(R,2);
tw=[2+1i 3+1i 4+1i 6+1i 7+1i 8+1i];%存储需要计算时间窗的工序（未转换前）
ET=[0 0 30 0 45 0 10 0 0 0 30 0 45 0 10];%（时间窗）
%%%%%%%%%%%this ET update should be noticed and improved.

afterorder=[2-1i 3-1i 4-1i 6-1i 7-1i  8-1i];%后序工位（用于转换前的解，避免出现2+、2-干扰的情况）
timewindow = numel(tw);
n = timewindow;
add=0;
priority=0;
...
for i = 1:line_num
    ...
    T_45(real(T_45) == unloading_station)=1+1i;
end

timesum = stationconvert_timesum (1,T_45);

for i = a(1) : b-1
    timing(nn)=timing(nn)+D(real(timesum(i)), real(timesum(i+1)));
end
...
if timing(nn)<ET(real(tt))
     waittime(nn)=ET(real(tt))-timing(nn);
end
end
