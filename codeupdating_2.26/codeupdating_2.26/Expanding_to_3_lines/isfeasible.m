...
minus = [2, 4, 6, 10, 12, 14, 18, 20, 22];  %*待优化
plus = [3 5 7 11 13 15 19 21 23];
check = [1, 2, 4, 6, 10, 12, 14, 18, 20, 22];%correlated to dual_feasible
dual_feasible = [0, 5, 0, 7, 0, 8, 0, 0, 0, 13, 0, 15, 0, 16, 0, 0, 0, 21, 0, 23, 0, 24];%2- to 3+, 3- to 6+, 4- to 5+, 5- to 7+.与minus对应&
...
%                end
  ...
  %进入可重入约束（reentrant）判断 (可精简化&)
  reentrant_ws = [6,7,14,15,22,23]; %可重入部分的工站编号
  first_ws = [2,3,10,11,18,19];  % 首次加工部分的工站编号
  if  feasible == 0
	% 第一个可重入工站
    pos1_begin = find(chrom == reentrant_ws(2)); %4+
    pos1_end = find(chrom == reentrant_ws(1)); %4-
     if pos1_end < pos1_begin
            chrom = [chrom(pos1_begin:n),chrom(1:(pos1_begin-1))]; %因加工次序是一个循环，可调整结构
            pos1_end = n - pos1_begin + pos1_end + 1; 
            pos1_begin = 1;   
     end
    chrom_re1 = chrom(:,pos1_begin:pos1_end); %4+~4—
    if (ismember(first_ws(1),chrom_re1)) || (ismember(first_ws(2),chrom_re1)) % 如果2-或2+在（4+，4-）之间
        feasible = 1;
     end
 end
  if  feasible == 0
  % 第二个可重入工站
    pos2_begin = find(chrom == reentrant_ws(4)); %8+
    pos2_end = find(chrom == reentrant_ws(3)); %8-
    if pos2_end < pos2_begin
            chrom = [chrom(pos2_begin:n),chrom(1:(pos2_begin-1))]; %因加工次序是一个循环，可调整结构
            pos2_end = n - pos2_begin + pos2_end + 1; 
            pos2_begin = 1;      
    end
    chrom_re2 = chrom(:,pos2_begin:pos2_end); % 8+~8-
    if (ismember(first_ws(3),chrom_re2)) || (ismember(first_ws(4),chrom_re2)) %如果6-或6+在（8+，8-）之间
        feasible = 1;
     end
  end
  if  feasible == 0
  % 第三个可重入工站
    pos3_begin = find(chrom == reentrant_ws(6)); %12+
    pos3_end = find(chrom == reentrant_ws(5)); %12-
    if pos3_end < pos3_begin
            chrom = [chrom(pos3_begin:n),chrom(1:(pos3_begin-1))]; %因加工次序是一个循环，可调整结构
            pos3_end = n - pos3_begin + pos3_end + 1; 
            pos3_begin = 1;      
    end
    chrom_re3 = chrom(:,pos3_begin:pos3_end); %12+~12-
    if (ismember(first_ws(5),chrom_re3)) || (ismember(first_ws(6),chrom_re3)) %如果10-或10+在（12+，12-）之间
        feasible = 1;
     end
  end
 ...
 if  feasible == 0
  ...
  if  feasible == 0
	% 第三个可重入工站
    pos3_begins = find(chrom ==  first_ws(6));
    pos3_ends = find(chrom ==  first_ws(5));
     if pos3_ends < pos3_begins
            chrom = [chrom(pos3_begins:n),chrom(1:(pos3_begins-1))]; %因加工次序是一个循环，可调整结构
            pos3_ends = n - pos3_begins + pos3_ends + 1; 
            pos3_begins = 1;            
     end
    chrom_re3s = chrom(:,pos3_begins:pos3_ends);
    if (ismember(reentrant_ws(5),chrom_re3s)) || (ismember(reentrant_ws(6),chrom_re3s))
        feasible = 1;
    end 
 end
 %可重入约束-------------------
