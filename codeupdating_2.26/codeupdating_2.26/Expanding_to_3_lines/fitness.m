...
solusion = R;
plus = [3 5 7 11 13 15 19 21 23];%当后序动作恰好在正后方时，用来加入等待时间（工位）&
ET=[0 0 30 0 45 0 10 0 0 0 30 0 45 0 10 0 0 0 30 0 45 0 10];%（时间窗）改进 &

line_num = 3;%两/三条线  （待改进&）
operation_num = 4;%每条线三个工序，加上最后返回卸载工站。  
for i = 1:line_num    %5和9、13是卸载站，赋值为1站
...
for j = 1:nind
 for i = 1 : numberofjourney
     len(j) = len(j) +  D(solusion(j,i), solusion(j,i+1)); % 根据实际情况此处的时间可以这样计算吗？
 end
